function json(s,p,l){
      $.getJSON(vhost+"/WeApi/Album/index/type/all",{key: key},function(res){
	  ...
	  $.getJSON(vhost+"/WeApi/Album",{
		...
		$.each(res.data,function (i, data){
			...
			$(".json-list").append('<li data-id="'+data.id+'" data-name="'+data.albumname+'">'+
				        '<div class="albumImg">'+
				          '<a href="/team/album/edit/mid/'+data.id+'.html" target="_blank">'+
				        ...
				        '<div class="meun">'+
				          '<a class="edit" href="/team/album/edit/mid/'+data.id+'.html">'+
...
$(document).on('click', '.dels',function(){
    ...
    layer.open({
      ...
      btn: ['确认','取消'],
      yes: function(index){
      	$.post("/team/album/album_trash",{id:id},function(d){
          if(d.code==0){
              TME.tips('删除成功');
              layer.close(index);
              history.go(0);
