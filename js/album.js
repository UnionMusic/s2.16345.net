$.getJSON(vhost+"/WeApi/Album/index/type/all",function(res){
	$("span.n1 sm").html(res.n0);
	$("span.n2 sm").html(res.n1);
	$("span.n3 sm").html(res.n2);
	$("span.n4 sm").html(res.n3);
	$("span.n5 sm").html(res.n4);
});
function json(s,p,l){
	  $.getJSON(vhost+"/WeApi/Album/index/sid/"+s+"/page/"+p+"/limit/"+l+"?tmp="+timestamp(),function(res) {
		$(".json-list").html("");
		$(".no-list").show();
		$.each(res.data,function (i, data){
			var sid = '<span></span>';
			if(data.sid==4){
				sid = '<span class="p4">未提交</span>';
			}else if(data.sid==0){
				sid = '<span>待审核</span>';
			}else if(data.sid==1){
			...
			$(".json-list").append('<li data-id="'+data.id+'" data-name="'+data.albumname+'">'+
				        '<div class="albumImg">'+
				          '<a href="/User/album/edit/mid/'+data.id+'.html" target="_blank">'+
				            '<img src="https://old.lianhe.art'+data.albumimg+'"></a>'+
				        '</div>'+
				        '<div class="albumName">'+data.albumname+sid+'</div>'+
				        '<div class="albumSinger">'+
				          '<i class="icon anticon icon-user"></i>'+data.singer.replace(/\+/g, '／')+'</div>'+
				        '<div class="albumTime">'+
				          '<i class="icon anticon icon-clockcircleo"></i>'+data.time.substr(0, 16)+'</div>'+
				        '<div class="meun">'+
				          '<a class="edit" href="/User/album/edit/mid/'+data.id+'.html">'+
...
function jsonstas(s){
	...
	json(meun, page, p);
		...
		function star(s){
			...
			$(".json-list").hide();
			$(".load-list").show();
			$.getJSON(vhost+'/WeApi/Album/index/sid/'+meun+'/type/count',function(res){
				c = res.count;
				t = res.page;
...
$(document).on('click', '.dels',function(){
    var r=confirm("确认删除《"+$(this).closest("li").attr("data-name")+"》吗？");
    if (r==true){
      $.post("/User/album/album_trash",{id:$(this).closest("li").attr("data-id")},function(d){
      	if(d.code==0){
        	TME.tips('删除成功');
          	history.go(0);
        }else{
