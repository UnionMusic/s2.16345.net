	...
	p.prototype.config = {
		...
		var n, o = this,
			...
			u = function() {
				var t = 0,
					n = 0,
					a = o.files || o.chooseFiles || r.files,  /* 去掉 e ||  修复拖拽无法获取正确ID*/
					u = function() {
			...
			d = function(e, i) {
				if (o.elemFile.next("." + s).remove(), r.value = "", "object" != typeof i) try {
					...
					return i = {}, TME.warn("请对上传接口返回有效JSON")
				}
		...
		if (v = 0 === v.length ? r.value.match(/[^\/\\]+\..+/g) || [] || "" : v, 0 !== v.length) {
			switch (l.accept) {
			case "file":
				if (h && !RegExp("\\w\\.(" + h + ")$", "i").test(escape(v))) return TME.warn("选择的文件中包含不支持的格式"), r.value = "";
				break;
			case "video":
				if (!RegExp("\\w\\.(" + (h || "avi|mp4|wma|rmvb|rm|flash|3gp|flv") + ")$", "i").test(escape(v))) return o.msg("选择的视频中包含不支持的格式"), r.value = "";
				break;
			case "audio":
				if (!RegExp("\\w\\.(" + (h || "mp3|wav|mid") + ")$", "i").test(escape(v))) return TME.warn("选择的音频中包含不支持的格式"), r.value = "";
				break;
			default:
				if (layui.each(v, function(e, i) {
					RegExp("\\w\\.(" + (h || "jpg|png|gif|bmp|jpeg$") + ")", "i").test(escape(i)) || (n = !0)
				}), n) return TME.warn("选择的图片中包含不支持的格式"), r.value = ""
			}
			...
			}(), l.number && o.fileLength > l.number) return TME.warn("同时最多只能上传的数量为：" + l.number);
